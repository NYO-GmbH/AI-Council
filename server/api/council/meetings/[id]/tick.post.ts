...
VOTE_RESULT: <example "Astra 3, Forge 2, Lumen 1">`,
  ...
  const summary = lines.find(line => line.startsWith('SUMMARY:'))?.replace('SUMMARY:', '').trim() || 'The council discussion is complete.'
  const winningIdea = lines.find(line => line.startsWith('WINNING_IDEA:'))?.replace('WINNING_IDEA:', '').trim() || 'No clear winning idea was identified.'
  const voteResult = lines.find(line => line.startsWith('VOTE_RESULT:'))?.replace('VOTE_RESULT:', '').trim() || 'No vote result available.'
...
export default defineEventHandler(async (event) => {
  ...
  if (activeMembers.length === 0) {
    throw createError({ statusCode: 400, statusMessage: 'No active council members available.' })
  }
  ...
  const currentState = meeting.state || { queue: [], rounds: 0, maxRounds: 2, concluded: false }
  const maxRounds = Math.max(1, currentState.maxRounds || 2)
  let queue = (currentState.queue || []).filter(memberId => activeMembers.some(member => member.id === memberId))
    ...
    if (rounds < maxRounds) {
      ...
      if (!currentState.concluded) {
        const fullTranscript = transcriptMessages
          .map(message => {
            const author = message.role === 'agent'
        ...
        await db.update(schema.councilMeetings).set({
          status: 'completed',
          state: {
            queue: [],
            rounds,
...
- Do not use markdown headings or bullet points.`,
  ...
  const [message] = await db.insert(schema.councilMessages).values({
    meetingId: meeting.id,
    memberId: speaker.id,
    role: 'agent',
    content
